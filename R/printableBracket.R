...
printableBracket <- function(bracket, add_seed=TRUE, add_prob=TRUE, w=NULL, cum_prob = TRUE, num_digits = 2){
  ...
  if(add_prob){
    if(cum_prob) {
    bracket[,teamname := paste0(teamname, ' (', sprintf(paste0("%1.", num_digits, "f%%"), 100*prob), ')')]
    } else {
      bracket[,teamname := paste0(teamname, ' (', sprintf(paste0("%1.", num_digits, "f%%"), 100*pred), ')')]
    }
  }
  ...
  addImg(obj = winnerLogo, x = winner$x, y = winner$y - 12, width = 20)
